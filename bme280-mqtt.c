...
// Compile: gcc -g -Wall -Wextra -pedantic -std=c11 -o bme280-mqtt bme280-mqtt.c -lpaho-mqtt3c
...
#define BME280_I2C_BUS "/dev/i2c-0"
...
void bme280_read() {
        // Create I2C bus
        int file;
        if((file = open(BME280_I2C_BUS, O_RDWR)) < 0) {
        ...
        char BUFFER[255];
	sprintf(BUFFER, "%.1f", cTemp);
        mqtt_send("/BME280/Temperature", BUFFER);
	//sprintf(BUFFER, "%.1f", humidity);
        //mqtt_send("/BME280/Humidity", BUFFER);
	sprintf(BUFFER, "%.1f", pressure/1.3332239);
        mqtt_send("/BME280/Pressure", BUFFER);
        close(file);
}
